...
com.config
├── core
│   ├── ConfigManager.java        # 主类
│   └── ConfigEntryProvider.java  # 枚举接口
├── entry
│   └── ConfigEntry.java          # 配置项定义（Builder 模式）
├── format
│   └── ConfigFormat.java         # JSON5 / YAML 枚举
├── type
│   ├── ConfigValueType.java      # 值类型枚举
│   ├── ConfigDuration.java       # 时间类型
│   └── ConfigDateTime.java       # 日期时间类型
└── util
    ├── ValueConverter.java       # 类型转换与范围校验
    ├── ConfigParser.java         # JSON5/YAML 解析器（→ flat map）
    └── ConfigSerializer.java     # flat map → JSON5/YAML 文本
```